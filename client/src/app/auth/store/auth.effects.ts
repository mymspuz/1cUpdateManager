...
export class AuthEffects {
  ...
  registerAction$ = this.actions$.pipe(
    ...
    switchMap(payload =>
      this.authService.register(
        ...
        map((res: any) => {
          const gravatarUrl = this.gravatarService.getUserGravatar(payload.user.email)
          const user = {
            uid: res.user.id,
            loginName: payload.user.login || res.user.loginName,
            email: res.user.email,
            photoUrl: res.user.photoURL || gravatarUrl,
            providerIn: res.user.providerIn,
        ...
        switchMap((user) => {
          return [
            ...
            new auth.UpdateProfile({loginName: payload.user.login, photoUrl: user.photoUrl}),
  ...
  updateProfile$ = this.actions$.pipe(
    ...
    switchMap((payload: any) => this.authService.updateProfile(payload.login, payload.photoUrl)
      .pipe(
        map(() => {
          ...
          return new auth.UpdateProfileSuccess({user: updateUser})
        }),
        catchError( (error) => of(new auth.AuthError(error)))
      )
    )
      ...
      .pipe(
        map((res: any) => {
          const user = {
            ...
            email: res.user.email,
            photoUrl: res.user.photoUrl,
            providerIn: res.additionalUserInfo.providerIn,
            isNewUser: res.additionalUserInfo.isNewUser
          }
          return user
        }),
        ...
        tap(() => this.router.navigateByUrl('')),
        catchError(error => {return of(new auth.AuthError({error}))})
      )
    )
  )
        ...
        map((authData: any) => {
          if (authData) {
            const user = {
              ...
              providerIn: authData.providerData[0].providerId,
